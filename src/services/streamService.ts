...
interface ApplicationAudioStream {
  stream: MediaStream
  cleanup: () => void
}

interface QueuedApplicationAudioChunk {
  left: Float32Array
  right: Float32Array
  offset: number
}
...
async function captureElectronApplicationAudio(sourceId: string): Promise<ApplicationAudioStream> {
  const api = typeof window !== 'undefined' ? window.electronAPI : undefined
  if (!api?.startApplicationAudioCapture || !api.stopApplicationAudioCapture || !api.onApplicationAudioData) {
    throw new Error('Application audio capture is not available.')
  }

  const session = await api.startApplicationAudioCapture(sourceId)
  const audioContext = new AudioContext({ sampleRate: session.sampleRate })
  const destination = audioContext.createMediaStreamDestination()
  const processor = audioContext.createScriptProcessor(4096, 0, Math.max(1, session.channels))
  const queuedChunks: QueuedApplicationAudioChunk[] = []
  let queuedFrames = 0
  let stopped = false

  const removeDataListener = api.onApplicationAudioData((captureId, chunk) => {
    if (stopped || captureId !== session.id || chunk.byteLength < 4) return
    ...
    for (let frame = 0; frame < frames; frame++) {
      const base = frame * 4
      left[frame] = Math.max(-1, view.getInt16(base, true) / 32768)
      right[frame] = Math.max(-1, view.getInt16(base + 2, true) / 32768)
    }

    queuedChunks.push({ left, right, offset: 0 })
    queuedFrames += frames

    const maxQueuedFrames = session.sampleRate * 2
    while (queuedFrames > maxQueuedFrames && queuedChunks.length > 1) {
      const dropped = queuedChunks.shift()
      if (dropped) queuedFrames -= dropped.left.length - dropped.offset
    }
  })

  const removeStoppedListener = api.onApplicationAudioStopped?.((captureId) => {
    if (captureId === session.id) stopped = true
  }) ?? (() => undefined)

  processor.onaudioprocess = (event) => {
    const leftOut = event.outputBuffer.getChannelData(0)
    const rightOut = event.outputBuffer.numberOfChannels > 1
      ? event.outputBuffer.getChannelData(1)
      : leftOut

    let written = 0
    while (written < leftOut.length && queuedChunks.length > 0) {
      ...
      rightOut.set(chunk.right.subarray(chunk.offset, chunk.offset + count), written)
      chunk.offset += count
      queuedFrames -= count
      written += count
  ...
  return {
    ...
    cleanup: () => {
      ...
      void api.stopApplicationAudioCapture(session.id).catch(() => undefined)
      void audioContext.close().catch(() => undefined)
    },
  }
}

function getElectronPlatform(): string | null {
  return typeof window !== 'undefined'
    ? window.electronAPI?.versionInfo?.platform ?? null
    : null
}
...
async function captureDisplayStream(
  ...
  const electronCapture = window.electronAPI?.setDesktopCaptureSource
  const isWindowsElectron = getElectronPlatform() === 'win32'
  const useElectronApplicationAudio = !!sourceId
    && !!electronCapture
    && sourceType === 'application'
    && audioMode === 'application'
    && isWindowsElectron
  const displayAudioMode = sourceId && electronCapture && sourceType === 'application'
    ? (useElectronApplicationAudio ? 'none' : audioMode)
    : audioMode

  if (sourceId && electronCapture) {
    await electronCapture({ sourceId, sourceType, audioMode: displayAudioMode })
  }

  let sourceStream: MediaStream
  try {
    sourceStream = await navigator.mediaDevices.getDisplayMedia(
      buildDisplayMediaOptions(sourceType, displayAudioMode, quality),
    )
  } finally {
    if (sourceId && electronCapture) {
      void electronCapture(null)
    }
  }

  if (useElectronApplicationAudio && sourceId) {
    let appAudio: ApplicationAudioStream | null = null
    const videoStream = sourceStream
    try {
      appAudio = await captureElectronApplicationAudio(sourceId)
      const audioTracks = appAudio.stream.getAudioTracks()
      if (audioTracks.length === 0) {
        throw new Error('Application audio capture produced no audio track.')
      }
      ...
      capturedStreamCleanups.set(sourceStream, () => {
        appAudio?.cleanup()
      ...
      throw error instanceof Error
        ? error
        : new Error('Application audio capture failed.')
    }
  }
...
export function getSupportedStreamAudioModes(sourceType: StreamSourceType): StreamAudioMode[] {
  if (typeof window !== 'undefined' && window.electronAPI?.getDesktopCaptureSources) {
