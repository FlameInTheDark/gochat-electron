...
describe('wsService presence', () => {
  ...
  it('marks the gateway ready only after the ready bootstrap event', async () => {
    const { socket } = await connectService()
    const { useGatewayConnectionStore } = await import('@/stores/gatewayConnectionStore')

    expect(useGatewayConnectionStore.getState().status).toBe('authenticated')
    expect(useGatewayConnectionStore.getState().ready).toBe(false)
    ...
    expect(useGatewayConnectionStore.getState().status).toBe('ready')
    expect(useGatewayConnectionStore.getState().ready).toBe(true)
  })
})
